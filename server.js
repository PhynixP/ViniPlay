...
function getSettings() {
    const defaultSettings = {
        ...
        streamProfiles: [
            ...
            { id: 'ffmpeg-intel', name: 'ffmpeg (Intel QSV)', command: '-user_agent "{userAgent}" -hwaccel qsv -hwaccel_output_format qsv -i "{streamUrl}" -vf vpp_qsv=w=1920:h=1080:format=nv12 -c:v h264_qsv -preset medium -global_quality 24 -profile:v high -level:v 4.2 -bf 0 -c:a aac -b:a 128k -f mpegts pipe:1', isDefault: false },
        ...
        castProfiles: [
            ...
            { id: 'cast-intel', name: 'Cast (Intel QSV)', command: '-user_agent "{userAgent}" -hwaccel qsv -hwaccel_output_format qsv -i "{streamUrl}" -vf vpp_qsv=w=1920:h=1080:format=nv12 -c:v h264_qsv -preset medium -global_quality 24 -profile:v high -level:v 4.2 -bf 0 -c:a aac -b:a 128k -movflags frag_keyframe+empty_moov+default_base_moof -f mp4 pipe:1', isDefault: false },
