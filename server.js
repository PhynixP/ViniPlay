    ...
    const defaultSettings = {
        ...
        dvr: {
            ...
            recordingProfiles: [
                ...
                { id: 'dvr-ts-intel', name: 'Intel QSV TS (Timeshiftable)', command: '-user_agent "{userAgent}" -hwaccel qsv -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i "{streamUrl}" -vf "format=nv12,hwupload=extra_hw_frames=64" -c:v h264_qsv -preset medium -global_quality 23 -c:a copy -f mpegts "{filePath}"', isDefault: false },
                { id: 'dvr-mp4-intel', name: 'Intel QSV MP4 (H.264/AAC)', command: '-user_agent "{userAgent}" -hwaccel qsv -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i "{streamUrl}" -vf "format=nv12,hwupload=extra_hw_frames=64" -c:v h264_qsv -preset medium -global_quality 23 -c:a aac -ac 2 -b:a 128k -movflags +faststart -f mp4 "{filePath}"', isDefault: false },
        ...
        castProfiles: [
            ...
            { id: 'cast-intel', name: 'Cast (Intel QSV)', command: '-user_agent "{userAgent}" -hwaccel qsv -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i "{streamUrl}" -vf "format=nv12,hwupload=extra_hw_frames=64" -c:v h264_qsv -preset medium -global_quality 24 -profile:v high -level:v 4.2 -bf 0 -c:a aac -b:a 128k -movflags frag_keyframe+empty_moov+default_base_moof -f mp4 pipe:1', isDefault: false },
